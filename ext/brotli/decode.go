...
func calculateDistanceAlphabetLimit(s *_State, maxDistance int32, npostfix int32, ndirect int32) int32 {
	...
	var offset = ((maxDistance - ndirect) >> uint(npostfix)) + 4
	var ndistbits = log2floor(offset) - 1
	var group = ((ndistbits - 1) << 1) | ((offset >> uint(ndistbits)) & 1)
...
func unpackCommandLookupTable(cmdLookup []int16) {
	var i int32
	var cmdCode int32
	var insertLengthOffsets = make([]int32, 24)
	var copyLengthOffsets = make([]int32, 24)
	...
	for cmdCode = 0; cmdCode < 704; cmdCode++ {
		...
		var insertCode = (((0x29850 >> uint(rangeIdx*2)) & 0x3) << 3) | ((cmdCode >> 3) & 7)
		var copyCode = (((0x26244 >> uint(rangeIdx*2)) & 0x3) << 3) | (cmdCode & 7)
		var copyLengthOffset = copyLengthOffsets[copyCode]
		var distanceContext = distanceContextOffset + min(copyLengthOffset, 5) - 2
		var index = cmdCode * 4
...
func initState(s *_State) int32 {
	...
	s.blockTrees[0] = 7
	s.distRbIdx = 3
	var result = calculateDistanceAlphabetLimit(s, 0x7FFFFFFC, 3, 120)
	if result < 0 {
		return result
	}
	var maxDistanceAlphabetLimit = result
...
func readSymbol(tableGroup []int32, tableIdx int32, s *_State) int32 {
	...
	var v = int32(shr64(s.accumulator64, s.bitOffset))
	offset += v & 0xFF
...
func inverseMoveToFrontTransform(v []int8, vLen int32) {
	var i int32
	var mtf = make([]int32, 256)
	for i = 0; i < 256; i++ {
		mtf[i] = i
	}
	for i = 0; i < vLen; i++ {
		var index = int32(v[i]) & 0xFF
		v[i] = int8(mtf[index])
		if index != 0 {
...
func readHuffmanCodeLengths(codeLengthCodeLengths []int32, numSymbols int32, codeLengths []int32, s *_State) int32 {
	...
	for symbol < numSymbols && space > 0 {
		...
		var p = (int32(shr64(s.accumulator64, s.bitOffset))) & 31
...
func readSimpleHuffmanCode(alphabetSizeMax int32, alphabetSizeLimit int32, tableGroup []int32, tableIdx int32, s *_State) int32 {
	var i int32
	var codeLengths = make([]int32, alphabetSizeLimit)
	var symbols = make([]int32, 4)
	...
	var result = checkDupes(s, symbols, numSymbols)
	if result < 0 {
		return result
	}
...
func readComplexHuffmanCode(alphabetSizeLimit int32, skip int32, tableGroup []int32, tableIdx int32, s *_State) int32 {
	var i int32
	var codeLengths = make([]int32, alphabetSizeLimit)
	var codeLengthCodeLengths = make([]int32, 18)
	var space int32 = 32
	...
	for i = skip; i < 18; i++ {
		...
		var p = (int32(shr64(s.accumulator64, s.bitOffset))) & 15
	...
	var result = readHuffmanCodeLengths(codeLengthCodeLengths, alphabetSizeLimit, codeLengths, s)
	if result < 0 {
		return result
	}
	return buildHuffmanTable(tableGroup, tableIdx, 8, codeLengths, alphabetSizeLimit)
...
func decodeContextMap(contextMapSize int32, contextMap []int8, s *_State) int32 {
	...
	var tableSize = maxHuffmanTableSize[(alphabetSize+31)>>5]
	var table = make([]int32, tableSize+1)
	var tableIdx = int32(len(table)) - 1
	...
	for i < contextMapSize {
		...
		if code == 0 {
			...
			var reps = (1 << uint(code)) + readFewBits(s, code)
			for reps != 0 {
				if i >= contextMapSize {
...
func decodeLiteralBlockSwitch(s *_State) {
	...
	var contextMode = int32(s.contextModes[literalBlockType])
...
func maybeReallocateRingBuffer(s *_State) {
	...
	var newBuffer = make([]int8, ringBufferSizeWithSlack)
	var oldBuffer = s.ringBuffer
...
func readMetablockPartition(s *_State, treeType int32, numBlockTypes int32) int32 {
	...
	var result = readHuffmanCode(blockTypeAlphabetSize, blockTypeAlphabetSize, s.blockTrees, 2*treeType, s)
	if result < 0 {
		return result
	}
	offset += result
...
func calculateDistanceLut(s *_State, alphabetSizeLimit int32) {
	...
	for i < alphabetSizeLimit {
		var base = ndirect + ((((2 + half) << uint(bits)) - 4) << uint(npostfix)) + 1
		for j = 0; j < postfix; j++ {
...
func readMetablockHuffmanCodesAndContextMaps(s *_State) int32 {
	...
	var result = readMetablockPartition(s, 0, s.numLiteralBlockTypes)
	if result < 0 {
		return result
	}
...
func huffmanTreeGroupAllocSize(alphabetSizeLimit int32, n int32) int32 {
	var maxTableSize = maxHuffmanTableSize[(alphabetSizeLimit+31)>>5]
...
func decodeHuffmanTreeGroup(alphabetSizeMax int32, alphabetSizeLimit int32, n int32, s *_State, group []int32) int32 {
	...
	for i = 0; i < n; i++ {
		group[i] = next
		var result = readHuffmanCode(alphabetSizeMax, alphabetSizeLimit, group, i, s)
		if result < 0 {
			return result
		}
		next += result
...
func doUseDictionary(s *_State, fence int32) int32 {
	...
	if address < 0 {
		var result = initializeCompoundDictionaryCopy(s, -address-1, s.copyLength)
		if result < 0 {
			return result
		}
		s.runningState = 14
		...
		var transformIdx = address >> uint(shift)
		offset += wordIdx * wordLength
		var transforms = rfcTransforms
...
func initializeCompoundDictionaryCopy(s *_State, address int32, length int32) int32 {
	...
	var index = int32(s.cdBlockMap[address>>uint(s.cdBlockBits)])
...
func decompress(s *_State) int32 {
	...
	for s.runningState != 10 {
		...
		case 4:
			...
			var insertAndCopyExtraBits = int32(cmdLookup[cmdCode])
			var insertLengthOffset = int32(cmdLookup[cmdCode+1])
			var copyLengthOffset = int32(cmdLookup[cmdCode+2])
		...
		case 7:
			if s.trivialLiteralContext != 0 {
				...
				var prevByte1 = int32(ringBuffer[(s.pos-1)&ringBufferMask]) & 0xFF
				var prevByte2 = int32(ringBuffer[(s.pos-2)&ringBufferMask]) & 0xFF
				for s.j < s.insertLength {
					if s.halfOffset > 1015 {
						result = readMoreInput(s)
						if result < 0 {
					...
					var literalTreeIdx = int32(s.contextMap[s.contextMapSlice+literalContext]) & 0xFF
					s.literalBlockLength--
					prevByte2 = prevByte1
					if s.bitOffset >= 32 {
						s.accumulator64 = (int64(s.intBuffer[s.halfOffset]) << 32) | shr64(s.accumulator64, 32)
			...
			if distanceCode < 0 {
				...
				var distTreeIdx = int32(s.distContextMap[s.distContextMapSlice+distanceCode]) & 0xFF
				distanceCode = readSymbol(s.distanceTreeGroup, distTreeIdx, s)
				if distanceCode < 16 {
					var index = (s.distRbIdx + distanceShortCodeIndexOffset[distanceCode]) & 0x3
					s.distance = s.rings[index] + distanceShortCodeValueOffset[distanceCode]
					if s.distance < 0 {
						return makeError(s, -12)
					}
				} else {
					var extraBits = int32(s.distExtraBits[distanceCode])
					var bits int32
...
func unpackTransforms(prefixSuffix []int8, prefixSuffixHeads []int32, transforms []int32, prefixSuffixSrc string, transformsSrc string) {
	var i int32
	var prefixSuffixBytes = toUtf8Runes(prefixSuffixSrc)
	var n = int32(len(prefixSuffixBytes))
	var index int32 = 1
	var j int32 = 0
	for i = 0; i < n; i++ {
		var c = prefixSuffixBytes[i]
		if c == 35 {
...
func transformDictionaryWord(dst []int8, dstOffset int32, src []int8, srcOffset int32, wordLen int32, transforms *_Transforms, transformIndex int32) int32 {
	...
	var dictOffset = srcOffset + omitFirst
	len -= omitFirst
	len -= omitLast
	var i = len
	for i > 0 {
	...
	if transformType == 10 || transformType == 11 {
		...
		for len > 0 {
			var c0 = int32(dst[uppercaseOffset]) & 0xFF
			if c0 < 0xC0 {
				if c0 >= 97 && c0 <= 122 {
		...
		var param = int32(transforms.params[transformIndex])
		var scalar = (param & 0x7FFF) + (0x1000000 - (param & 0x8000))
		for len > 0 {
			var step int32 = 1
			var c0 = int32(dst[shiftOffset]) & 0xFF
			if c0 < 0x80 {
				scalar += c0
				...
				if len >= 2 {
					var c1 = int32(dst[shiftOffset+1])
				...
				if len >= 3 {
					var c1 = int32(dst[shiftOffset+1])
					var c2 = int32(dst[shiftOffset+2])
				...
				if len >= 4 {
					var c1 = int32(dst[shiftOffset+1])
					var c2 = int32(dst[shiftOffset+2])
					var c3 = int32(dst[shiftOffset+3])
...
func buildHuffmanTable(tableGroup []int32, tableIdx int32, rootBits int32, codeLengths []int32, codeLengthsSize int32) int32 {
	...
	var sorted = make([]int32, codeLengthsSize)
	var count = make([]int32, 16)
	var offset = make([]int32, 16)
...
func readMoreInput(s *_State) int32 {
	...
	for bytesInBuffer < 4096 {
		...
		var len = readInput(s, s.byteBuffer, bytesInBuffer, spaceLeft)
		if len < -1 {
			return len
		}
		if len <= 0 {
...
func checkHealth(s *_State, endOfStream int32) int32 {
	...
	var byteOffset = (s.halfOffset << 2) + ((s.bitOffset + 7) >> 3) - 8
...
func readFewBits(s *_State, n int32) int32 {
	var v = (int32(shr64(s.accumulator64, s.bitOffset))) & ((1 << uint(n)) - 1)
	s.bitOffset += n
	return v
}
...
func copyRawBytes(s *_State, data []int8, offset int32, length int32) int32 {
	...
	var copyNibbles = min(halfAvailable(s), len>>2)
	if copyNibbles > 0 {
		var readOffset = s.halfOffset << 2
		var delta = copyNibbles << 2
	...
	for len > 0 {
		var chunkLen = readInput(s, data, pos, len)
		if len < -1 {
			return len
		}
		if chunkLen <= 0 {
...
func unpackDictionaryData(dictionary []int8, data0 string, data1 string, skipFlip string, sizeBits []int32, sizeBitsData string) {
	...
	var skipFlipRunes = toUtf8Runes(skipFlip)
	var offset int32 = 0
	var n = int32(len(skipFlipRunes)) >> 1
	for i = 0; i < n; i++ {
		var skip = skipFlipRunes[2*i] - 36
		var flip = skipFlipRunes[2*i+1] - 36
		for j = 0; j < skip; j++ {
...
func init() {
	var dictionaryData = make([]int8, 122784)
	var dictionarySizeBits = make([]int32, 25)
